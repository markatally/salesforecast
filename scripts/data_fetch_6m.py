"""Fetch monthly product-demand input data for n6m-jenny.

The defaults follow the "产品中期需求预测取数" SQL in the handover document.
They can be overridden without editing SQL text.
"""
...
DEFAULT_PRODUCT_CODES = ("162", "169", "193", "170", "171")
DEFAULT_FROM_TYPES = ("经销商", "零售")
DEFAULT_TERMINAL_TYPE = "医疗机构"
TABLE_SCOPES = (
...
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "data" / "sales_6m_monthly.csv"
...
def _build_scope_sql(
    ...
    conditions = [
        ...
        f"ddpmfs.tomdmtype1 = {_quote_sql(terminal_type)}",
    ...
    return f"""
    SELECT
        ddpmfs.bizym, ddpmfs.prodmdmcode, ddpmfs.tomdphncode,
        SUM(ddpmfs.cnvrtdqty) AS qty
    FROM {table} ddpmfs
    WHERE {where_sql}
    GROUP BY ddpmfs.bizym, ddpmfs.prodmdmcode, ddpmfs.tomdphncode
    """.strip()
...
def _build_sarx_sql(
    ...
    conditions = [
        ...
        f"sarx.tomdmtype1 = {_quote_sql(terminal_type)}",
    ...
    return f"""
    SELECT
        sarx.bizym, sarx.prodmdmcode, sarx.tomdphncode,
        SUM(sarx.cnvrtdqty) AS qty
    FROM {SARX_TABLE} sarx
    WHERE {where_sql}
    GROUP BY sarx.bizym, sarx.prodmdmcode, sarx.tomdphncode
    """.strip()
...
def build_sql(
    ...
    sql = "SELECT *\nFROM (\n    " + "\n    UNION ALL\n    ".join(scopes) + "\n)\nORDER BY bizym, prodmdmcode, tomdphncode"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch monthly input data for n6m-jenny.")
